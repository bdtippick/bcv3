...
-- 헬퍼 함수: 현재 사용자의 역할, 회사ID, 지점ID를 가져옴 (수정된 버전)
...
-- 대안: user_profiles 테이블을 직접 조회하는 방식
CREATE OR REPLACE FUNCTION get_current_user_info_from_profile()
RETURNS TABLE(role TEXT, company_id UUID, branch_id UUID) AS $$
  SELECT
    up.role::TEXT,
    up.company_id,
...
-- 정책: companies (user_profiles 조회 방식 사용)
...
-- 정책: user_profiles (자기 자신의 프로필 조회 허용)
    ...
    WITH CHECK (
      ...
      CASE 
        ...
        WHEN EXISTS (
          ...
          WHERE up.id = auth.uid() 
            AND up.role = 'branch_manager' 
            AND up.branch_id = riders.branch_id
        ) THEN true
        ELSE false
      END
    );
    ...
    WITH CHECK (
      ...
      CASE 
        ...
        WHEN EXISTS (
          ...
          WHERE up.id = auth.uid() 
            AND up.role = 'branch_manager' 
            AND up.branch_id = settlement_periods.branch_id
        ) THEN true
        ELSE false
      END
    );
    ...
    FOR SELECT USING (
        EXISTS (
            ...
            WHERE sp.id = settlement_records.settlement_period_id
              AND sp.company_id = up.company_id
              AND (
                -- 회사 관리자는 모든 지점 조회 가능
                up.role IN ('company_admin', 'super_admin') OR
                -- 지점 관리자는 자기 지점만 조회 가능
                (up.role = 'branch_manager' AND sp.branch_id = up.branch_id)
              )
        )
    );
